...
- **Afiliaciones incompletas.** El encabezado (L2) tiene superíndices numéricos (2,3,4,5) pero **no hay lista de afiliaciones** en el texto. No se puede verificar filiación institucional ni correspondencia más allá del símbolo ✉.
  > **Corrección (ronda 2 — verificado / parcialmente artefacto):** En el Doc real, René Lagos figura como `René Lagos[1]` con afiliación en **nota al pie** (`#ftnt1`); la extracción de texto descarta el marcador. Las afiliaciones institucionales se implementan como **notas al pie**, que esta extracción **no surface**, de modo que su contenido **no es verificable** por esta vía — NO se afirma que estén ausentes. Verificar sobre el export a Word/PDF que rendericen como lista de afiliaciones estándar. *(Los otros componentes de M5 —COI, CRediT, ORCID— no son capa de presentación y se mantienen intactos como defectos de integridad reales.)*
...
> **Corrección (ronda 2 — verificado contra la lista de referencias REAL):** Este defecto **se CONFIRMA como REAL**, no es artefacto de extracción. Se cotejó cada cita contra la Sección 5 completa del Doc real (entradas de primer autor: Aguilera, Amouzou, Carrasco, Carvalho, Cilia, Corsi Sotelo, Ferlay, Ferreccio, Frenz, Gouveia, Hart, Lagos Barrios & Batarce, Lagos & Cuadrado, Lagos & Jofré, Lagos et al. 2026, Lagos et al. En revisión, Latorre 2015, Libuy, Lozano, Marsh, Martínez, MINSAL 2025, Quezada-Diaz, Riquelme, Shengelia, Silva, Superintendencia, Tanahashi, Taramasco). **Ninguna** de Chatignoux 2021, Heise 2009, Navarrete 2018, Latorre 2024, Thiruvengadam 2024 ni Leslie 2019 aparece como entrada de primer autor. Matices: *Latorre 2015* SÍ está (≠ Latorre 2024); *Leslie H.H.* aparece solo como **coautora** de *Amouzou et al. 2019*. Se mantiene íntegra la exigencia.
...
- **m3. Figuras y varios anexos son marcadores `[image]` vacíos.** Figuras 1 y 2 (L200–211) y Anexos 0, 3, 5 y 6 aparecen como `[image]` sin contenido. El revisor no puede verificar los resultados visuales (cascadas, tendencias, ajuste del error de incidencia). Reproducibilidad de resultados gráficos: no evaluable.
  > **Corrección (ronda 2 — verificado contra el Doc real):** **ARTEFACTO de extracción**, no ausencia. El `[image]` es cómo un export de texto representa un objeto de imagen embebido; las figuras existen (los comentarios internos de coautores discuten su layout y apariencia renderizados: *"Se ven mucho mejor! Probaria con los IC95%…"*, *"Plotealos como 4×1…"*). La **reproducibilidad gráfica** se evalúa sobre la figura renderizada/el código que la genera (repo `GastricCancerEffectiveCoverage`), no sobre el marcador de texto. Se retira "no evaluable por ausencia"; lo pertinente a reproducibilidad es que el **código y datos que generan las figuras** sean depositados (ya cubierto en M3 principal).
- **m4. Anexo 4 truncado.** La tabla de EC por red/sexo/edad (Anexo 4) se corta en "Metropolitano Central" (L572) y no cubre todas las 45 redes. La verificación red-por-red de las cifras es imposible tal como está.
  > **Corrección (ronda 2 — parcial / NO VERIFICABLE, probable artefacto):** Una relectura directa del Doc real vía API **también** corta el Anexo 4 en "Metropolitano Central" (mismo punto que la extracción original), y la herramienta advierte explícitamente que *"el contenido puede estar incompleto para archivos muy grandes"*. El Anexo 4 es la tabla más grande del documento (45 redes × sexo × grupo etario ≈ cientos de filas), el candidato natural a truncamiento de serialización. **Crucialmente, los datos de todas las redes SÍ existen en el estudio:** la Tabla 4 del cuerpo lista redes del sur con valores completos (Osorno, Aysén, Magallanes, Chiloé, Metropolitano Oriente/Sur) y el Anexo 2 lista todas las macro-regiones. Por tanto **no falta contenido del estudio**; lo que no puedo confirmar por extracción de texto es si el Anexo 4 renderiza las 45 redes en el Doc real. **Se degrada** de "el doc trunca el Anexo 4" a "no verificable por extracción; verificar sobre el export/PDF; datos por red presentes en Tabla 4 / Anexo 2".
