...
> **Corrección (ronda 2 — verificado contra el Google Doc real, ID `1cJU…AIc_A`):** Las figuras **SÍ existen** en el manuscrito; NO son placeholders. El token `[image]` es un **artefacto de extracción de texto**: tanto la extracción original como la lectura directa del Doc vía la API de Google Drive representan todo objeto de imagen embebido como `[image]` (un export de texto no puede renderizar imágenes). La evidencia decisiva son los **comentarios internos de coautores sobre las figuras renderizadas**: *"Se ven mucho mejor! Probaria con los IC95%… graficos lindos con ggplot2 de series de tiempo"*; *"Plotealos como 4×1 en vez de 1×4… Agrega legendas claras… Incluye una legenda explicativa suficiente en el pie de la figura"*; y *"Hacer una imagen más bonita con Claude de calidad publicacion"*. No se comenta el layout ni la apariencia de una figura inexistente. Figuras 1(a,b), 2(a,b) y los Anexos 0 / 3 / 5(a-c) / 6(a,b) están presentes como imágenes.
> **Reclasificación de severidad:** M1 baja de **"no existe una sola figura real → rechazo por presentación"** a **"figuras presentes; ajustes de forma"** (orientación 4×1, ribbon de IC95%, leyendas autoexplicativas). La **calidad se evalúa sobre la figura renderizada**, no sobre la extracción de texto; con la evidencia disponible NO se afirma insuficiencia de calidad. Las exigencias de forma de arriba se mantienen como mejora y coinciden con lo que los propios coautores ya pidieron en los comentarios.
...
> **Corrección (ronda 2):** La parte (a) —leyendas telegráficas— es un ajuste de **forma sobre figuras que existen** (ver corrección de M1), no una figura ausente; coincide con el comentario interno que pide "legenda explicativa suficiente en el pie de la figura". La parte (b) —brecha entre la cascada de 7 niveles de la Tabla 1 y las 4 métricas operacionalizadas (AC/UC/Q/EC)— es un punto **de contenido/sustantivo** verificado sobre texto extraído correctamente (Tabla 1 muestra la cascada Target→…→Effective) y consolidado como **C16** en el informe; **se mantiene intacto** y NO es un artefacto de presentación.
...
> **Corrección (ronda 2 — verificado contra el Doc real):** La "sopa de entidades" `&#25;…&#30;` es un **artefacto de extracción**, NO una ecuación corrupta. En el Doc real las ecuaciones son objetos LaTeX bien formados que renderizan correctamente; la extracción las duplica: primero el LaTeX limpio y luego un volcado del flujo interno de glifos del objeto-ecuación (caracteres de uso privado `&#25;` `&#27;` `&#29;` `&#30;` del editor de ecuaciones de Google). Ejemplos verificados en el Doc: la ecuación de Calidad aparece como `${Q}_{isat}=\frac{({HG}_{sa}^{min}\ -\ {HG}_{isat}\ )}{({HG}_{sa}^{min}\ -\ {HG}_{sa}^{max})}$` (seguida de la sopa), y también `${EC}_{isat}=\frac{{U}_{isat}}{{N}_{isat}}\cdot {Q}_{isat}$`, `${EC}_{it}=\frac{\sum_{sa}{EC}_{isat}\cdot N_{isat}}{\sum_{sa}N_{isat}}$`, `${UC}_{it}$`, `${AC}_{it}$`, `$\widehat{{IMR}_{sa}}$`. Un comentario interno confirma "Usa ecuaciones de una vez" (ya migradas a objetos-ecuación). **Reclasificación:** la ilegibilidad de ecuaciones baja de defecto MAYOR a **artefacto de extracción**. Lo que SÍ sobrevive y se mantiene como punto real (menor, de notación) es la **colisión notacional** `j=1` (índice de intervención vs. indicador de necesidad `N=1`): desambiguar sigue siendo recomendable.
...
> **Corrección (ronda 2):** Las anclas `<comment_start id=kix…>`, los enlaces `zotero.org/google-docs/?…` (field codes de Zotero) y las anclas de comentario son **capa colaborativa viva de Google Docs que desaparece al exportar a Word/PDF** (los comentarios se descartan; los field codes de Zotero se aplanan a citas con formato + bibliografía). Es **higiene de exportación, NO un defecto de contenido del manuscrito**, y NO justifica un desk-reject: el editor recibe el PDF/Word exportado, no el Doc con capa de edición. Se mantiene la recomendación de aceptar/rechazar cambios y limpiar comentarios **al exportar**. Nota real que sí persiste: hay al menos un texto **tachado** (`~~…~~`) al cierre de la Introducción (tracked-change pendiente de aceptar). La verificación de la bibliografía por DOI se hace sobre el export con Zotero desconectado, no sobre los placeholder links.
...
> **Corrección (ronda 2 — verificado contra el Doc real):** El sub-punto de las **filas de encabezado vacías `| | | |`** es un **artefacto de serialización de tablas de Google Docs a Markdown**: Docs no marca una "fila de encabezado" semántica, así que el serializador emite una fila vacía + separador y coloca el encabezado real (en negrita) como primera fila del cuerpo. En el Doc renderizado NO hay fila fantasma. **Se retira** como defecto. Los demás sub-puntos **se mantienen como REALES** (sobreviven a la extracción y se verificaron en el Doc): (i) **Anexo 2 íntegramente en español** — confirmado, encabezado real *"Macro Region | Nombre | Poblacion Fonasa | … | Confirmacions CG | … | Defunciones Fonasa CG"* (con "Confirmacions" mal escrito); (ii) **Tabla 2 con "Maximum health gain (HGmax)" duplicado** — confirmado, la 2ª fila ("99th percentile of HMR") debe ser **HGmin**; (iii) la doble etiqueta "AC" con escalas distintas (j=1 ≈0,01-0,04 vs j=2 ≈90-430) sin unidades persiste como asunto de contenido. El *spanning* de encabezados de grupo sí se pierde en la serialización, pero la falta de unidades por columna es real.
...
- **m18.** Autores (L2): *"René Lagos,2, Cristóbal Cuadrado3,2,✉, Arnoldo Riquelme4,5,2"* — **superíndices de afiliación rotos** (René Lagos sin afiliación "1"), **sin lista de afiliaciones**, símbolo de autor de correspondencia (✉) sin dirección.
  > **Corrección (ronda 2 — verificado):** En el Doc real, **René Lagos SÍ tiene afiliación**: figura como `René Lagos[1]` con la afiliación en una **nota al pie** (`#ftnt1`); la extracción de texto descarta el marcador `[1]` y lo deja como `,2`, de ahí la apariencia de "línea de autores rota". Los superíndices 2,3,4,5 y el símbolo ✉ están presentes. La **lista de afiliaciones institucionales se implementa como notas al pie**, que esta extracción de texto **no surface**, por lo que su contenido **no es verificable** por esta vía — NO se afirma que las afiliaciones estén ausentes. Recomendación: verificar sobre el export a Word/PDF que las notas al pie de afiliación rendericen como lista estándar (institución por superíndice) y que la dirección de correspondencia acompañe al ✉.
...
- **m20.** Citas **no gestionadas** (texto plano, sin enlace) mezcladas con las de Zotero: *"(Heise et al 2009)"* (L58), *"(Navarrete et al 2018)"* (L224), *"(Latorre et al., 2024; Thiruvengadam et al., 2024)"* y *"(Leslie et al., 2019; Lozano et al., 2020)"* (L232) — **varias sin entrada en la lista de referencias** (Heise 2009, Navarrete 2018, Latorre 2024, Thiruvengadam 2024, Leslie 2019 no aparecen en las referencias A-T). Citas colgantes.
  > **Corrección (ronda 2 — verificado contra la lista de referencias REAL):** Este punto **se CONFIRMA como REAL**, no es artefacto. Se cotejó cada cita contra la Sección 5 (References) del Doc real: **Heise (2009), Chatignoux (2021), Navarrete (2018), Latorre (2024), Thiruvengadam (2024) y Leslie (2019) NO tienen entrada de primer autor en la lista.** Precisiones: *Latorre 2015* SÍ está (pero *Latorre 2024* no); *Leslie* aparece solo como **coautora** de *Amouzou et al. 2019 (BMJ Global Health)*, no como entrada de primer autor 2019; *Chatignoux (2021)* se cita en Anexo 0 y tampoco figura. Las citas de texto plano no gestionadas por Zotero (Heise, Navarrete, Chatignoux) confirman que quedaron fuera del gestor bibliográfico. **Defecto de integridad bibliográfica real; se mantiene la exigencia** de completar cada entrada con DOI y verificar el par cita↔referencia.
...
> **Corrección (ronda 2 — verificado contra el Doc real):** Los dos pilares más fuertes de este ataque —"figuras son todas cuadros vacíos `[image]`" y "ecuaciones aparecen como `&#25;(&#25;HG…`"— son **artefactos de extracción**, no defectos del documento: las figuras existen (los coautores comentan su layout renderizado) y las ecuaciones están como objetos LaTeX bien formados. También los "enlaces muertos zotero.org/google-docs" son field codes que se aplanan al exportar. **Lo que sí sostiene el reproche** —y persiste tras cualquier exportación— es de nivel forma/texto: el abstract con oraciones rotas (*"needs to to transition"*, *"The results highlight a persistent gap remains"*), el **anexo entero en español** (*"Confirmacions CG", "Defunciones"*), y varias **referencias del texto ausentes de la lista** (verificado REAL). El diagnóstico "no puedo evaluarlo porque la mitad de la evidencia es inexistente" **ya no aplica**: sobre el export, figuras y ecuaciones son revisables.
...
> **Corrección de severidad (ronda 2 — verificado contra el Doc real):** Dos de los cinco pilares que sostenían la advertencia de *desk-reject por presentación* se disuelven al verificar contra el Doc real: **figuras 100% ausentes (M1)** y **ecuaciones corruptas (M3)** eran **artefactos de extracción** — las figuras existen y las ecuaciones renderizan. Los **residuos editoriales/Zotero (M4)** son higiene de exportación, no defecto de contenido; y las **filas de encabezado fantasma (M5)** son artefacto de serialización. Lo que **permanece REAL** y sostiene "grandes revisiones" es de nivel forma/texto y contenido: **Anexo 2 en español**, **HGmax→HGmin** en Tabla 2, **~27 erratas de inglés** y **abstract con oraciones rotas** (verificados), la **inconsistencia de siglas** (M6), las **referencias citadas ausentes de la lista** (m20, verificado REAL) y la brecha **framework↔métricas** (M2b, sustantiva = C16). En consecuencia, **ya NO corresponde la advertencia de "rechazo en mesa editorial por presentación"**: la presentación exportada es revisable. La severidad de este panelista se mantiene en **GRANDES REVISIONES**, ahora sustentada en forma/lenguaje/coherencia y no en figuras/ecuaciones inexistentes. Condiciones de reenvío revisadas: (1) figuras **con leyendas autoexplicativas e IC95%** (ya existen; es pulido, no creación); (2) ecuaciones ya como objetos-ecuación — verificar el render en el export; (3) limpieza de comentarios/tracked-changes/field-codes **al exportar**; (4) alineación framework-métricas; (5) glosario y nomenclatura única; (6) pase de corrección de estilo; (7) completar las referencias faltantes con DOI.
