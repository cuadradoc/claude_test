...
| 2 | search: default tier returns deduplicated results | PASS | 19 unique from 20 raw hits; live sources: crossref, europepmc, openalex, pubmed | 16.2 |
...
| 6 | search: a failing source degrades gracefully | PASS | survived with 9 papers; errors recorded: none | 2.3 |
| 7 | search: nonsense query is flagged as low relevance | PASS | 3 off-topic results returned by PubMed, all flagged, warning shown | 1.7 |
...
| 10 | doi: resolves a real DOI to metadata | PASS | resolved: worldwide trends in body-mass index, underweight, overweight, and obesity from 1 | 1.7 |
| 11 | download: Sci-Hub fallback is on by default, --no-scihub disables it | PASS | default chain is native -> OA repositories -> Unpaywall -> Sci-Hub (order verified upstream); --no-scihub stops after Unpaywall | 3.1 |
...
| 2 | read: extracts full text from a PDF | PASS | 284,850 chars extracted | 2.7 |
| 3 | doctor: reports per-source availability | PASS | 15 OK, 6 empty, 0 error (of 21 sources) | 17.3 |
...
15/21 sources returned results for "obesity".
Polite-pool email: set
